...
smo_options = before_options_defined({})

if len(victory_names) > 1:
    goal = {'option_' + v: i for i, v in enumerate(victory_names)}
    smo_options['goal'] = type('goal', (Choice,), goal)
    smo_options['goal'].__doc__ = "Choose your victory condition."

if any(item.get('trap') for item in item_table):
    smo_options["filler_traps"] = FillerTrapPercent

if game_table.get("death_link"):
    smo_options["death_link"] = DeathLink

for category in category_table:
    for option_name in category_table[category].get("yaml_option", []):
        if option_name[0] == "!":
            option_name = option_name[1:]
        if option_name not in smo_options:
            smo_options[option_name] = type(option_name, (DefaultOnToggle,), {"default": True})
            smo_options[option_name].__doc__ = "Should items/locations linked to this option be enabled?"

smo_options = after_options_defined(smo_options)
smo_options_data = make_dataclass('SMOOptionsClass', smo_options.items(), bases=(PerGameCommonOptions,))
