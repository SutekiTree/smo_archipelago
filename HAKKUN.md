...
| 3b (partial, in progress) | `c60052a` | 23 trampolines installed (Scenario/MoonGet/Death/ShineNumGet/ShineNumByWorld/AddHackDictionary/AddPayShine×2/CaptureStart/WorldMap×4/MoonLabel×3/ShineAppearance/CreditsStart×Strategy B/CappyMessage×4/SaveLoad + GameSystemInit/DrawMain); game/CaptureGate/KingdomUnlock/KingdomOrderGate/MoonApply ported; ui/CappyMessenger/ApHudOverlay ported; ApState applyOnFrame/flushPendingCaptureGrants/maybeApplyInboundKill/buildPaySnapshot/setPendingMoonLabel/tryTakePendingMoonLabel implemented; ApClient is a MINIMAL STUB (no socket/thread/nifm yet — see Phase 3b residual below). subsdk8 ~73 KiB; sail resolves 48 SMO symbols. |
| 3b (residual: ApClient.cpp port) | _TBD_ | Final piece: port ApClient's 1124-line networking core. nn::socket → hk::socket (BSD-like API via `hk::socket::Socket::initialize<"bsd:u">` + Socket+Connect+Send+Recv+Close+poll); nn::nifm → add `switch-mod-hk/syms/nn/nifm.sym` with `_ZN2nn4nifm10InitializeEv` / `_ZN2nn4nifm28SubmitNetworkRequestAndWaitEv` / `_ZN2nn4nifm19IsNetworkAvailableEv` then call via sail-resolved fn ptrs; nn::os::CreateThread → `hk::os::Thread` with page-aligned stack; drop the manual sockaddr workaround in favor of `hk::socket::SocketAddrIpv4::parse`. Lambda body / reconnection backoff / line buffering / handleLine dispatch all port verbatim. |
